"""Full-budget MERLIN input-separation run; compare with all saved runs."""
import os
import sys
...
cfg = TrainConfig(iters=700, ri_mode="merlin")
res = denoise(amp, cfg, ri_pair=ri)
np.save(os.path.join(OUT, "denoised_MERLIN.npy"), res["denoised"])

runs = {
    "baseline": np.load(os.path.join(OUT, "denoised_baseline.npy")),
    "+RI": np.load(os.path.join(OUT, "denoised_RI.npy")),
    "+RI-tuned": np.load(os.path.join(OUT, "denoised_RI_tuned.npy")),
    "MERLIN": res["denoised"],
}
...
for r in rows:
    ...
    print(line)
    lines.append(line)
with open(os.path.join(OUT, "metrics_merlin.txt"), "w") as f:
    f.write("\n".join(lines) + "\n")
...
names = ["Noisy", "baseline", "+RI", "MERLIN"]
imgs = [amp, runs["baseline"], runs["+RI"], runs["MERLIN"]]
...
fig.tight_layout()
fig.savefig(os.path.join(OUT, "compare_merlin.png"), dpi=130)
print("Saved", os.path.join(OUT, "compare_merlin.png"))
