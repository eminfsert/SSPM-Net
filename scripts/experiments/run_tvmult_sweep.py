"""Dedicated tv_mult sweep on the NEW pixel+group base (open lead from
2026-08-31: on the old band+batch base ratio-ENL preferred tv_mult=7.9
while ENL-ROI preferred 10 — and every stack knob was tuned on that old
base, so the operating point must be re-checked after the A1/A2 win).

Protocols identical to run_arch_ablation.py:
  synthetic-GT : known clean + simulated 1-look complex speckle with
                 reciprocity physics (seed 7, sigma_n bisection-calibrated)
  real patch   : noisy-reference table incl. ratio-ENL (ideal ~= 1)

All runs .npy-cached (interrupted Colab sessions resume).
"""
import os, sys
import numpy as np
...
for tv in TVS:
    ...
    runs_s[name] = cached(f"tvsweep_synth_{tag}.npy", lambda: denoise(
        amp_sim, TrainConfig(**STACK, tv_mult=tv, whiteness_lags=(1, 2, 3),
                             model_cfg=PG),
        ri_pair=ri_sim, pha=maps_sim)["denoised"]).astype(np.float64)
    print(f"\n=== real {name} ===", flush=True)
    runs_r[name] = cached(f"tvsweep_real_{tag}.npy", lambda: denoise(
        amp, TrainConfig(**STACK, tv_mult=tv, whiteness_lags=(3, 4, 5),
                         model_cfg=PG),
...
with open(f"{OUT}/metrics_tvmult_sweep.txt", "w") as f:
    f.write("\n".join(lines) + "\n")

import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
names = ["Noisy"] + list(runs_r.keys())
imgs = [amp] + list(runs_r.values())
...
fig.tight_layout()
fig.savefig(f"{OUT}/compare_tvmult_sweep.png", dpi=130)
print("\nSaved", f"{OUT}/compare_tvmult_sweep.png")
